...
import { personalInfo, educationData, certificationsData, projectsData, skillsData } from '../data/portfolioData';
import { downloadResume } from '../utils/downloadResume';

export default function ResumeModal({ theme, onClose }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fadeIn">
      ...
      <div 
        className={`relative w-full max-w-4xl max-h-[92vh] flex flex-col rounded-3xl border shadow-2xl overflow-hidden ${
          isDark 
            ? 'bg-slate-900 border-slate-700 shadow-sky-500/10' 
            : 'bg-white border-amber-300 shadow-amber-900/15'
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Top Modal Controls */}
        <div className={`flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b ${
          isDark ? 'bg-slate-950 border-slate-800' : 'bg-amber-50/90 border-amber-200'
        }`}>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-xs text-white transition-all shadow-md active:scale-95 ${
                isDark
        ...
        <div className={`overflow-y-auto p-6 sm:p-10 space-y-8 font-sans ${
          ...
          <div className="space-y-2">
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs ${
              ...
              <div><strong className={isDark ? "text-white" : "text-slate-900"}>Frontend:</strong> React.js, Tailwind CSS, Bootstrap, HTML5, CSS3</div>
            ...
            <div className="space-y-3">
              {educationData.map((edu, idx) => (
                <div key={idx} className="flex justify-between items-start text-xs">
                  <div>
                    <div className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{edu.degree}</div>
          ...
          <div className="space-y-3">
            ...
            {certificationsData.map((cert, idx) => (
              <div key={idx} className="flex flex-wrap justify-between items-center text-xs gap-2 py-1">
                <div>
                  <span className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{cert.title}</span> — <span className={isDark ? "text-slate-400" : "text-slate-600"}>{cert.issuer}</span>
                  {cert.credentialId && (
